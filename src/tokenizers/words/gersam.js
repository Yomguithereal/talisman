/**
 * Talisman tokenizers/words/gersam
 * =================================
 *
 * A language-configurable simple word tokenizer inspird by the Moses
 * machine translation system's tokenizer.
 *
 * It is called Gersam because Gersam is the Latin name of Moses' firstborn son.
 *
 * [Urls]:
...
const SPECIAL_SEPARATOR = new RegExp(`([^${NUMBER}${LETTER}\\s.’'\`,\\-${MISC_SYMBOLS}])`, 'gu');
 ...
 *
 * @param  {string} lang - Target language in ISO 639-1 format.
 * @param  {string} text - The text to tokenize.
 * @return {array}       - The tokens.
 */
export default function tokenize(lang, text) {
  ...
  // Protecting abbrevations
  text = text.replace(ABBREV, m => {
    return m.replace(ABBREV_POINT, 'KTZABBREVZTK');
  });

  // Separating special characters
  if (lang === 'fi' || lang === 'sv') {
    text = text.replace(FI_COLON1, ' $1 ');
    text = text.replace(FI_COLON2, ' $1 ');
  }
  else {
    text = text.replace(SPECIAL_SEPARATOR, ' $1 ');
  }

  // Separating comma
  text = text.replace(COMMA_SEPARATOR1, '$1 $2 ');
  text = text.replace(COMMA_SEPARATOR2, ' $2 $1');
  text = text.replace(COMMA_SEPARATOR3, '$1 $2');

  text = text.replace(POINT_SEPARATOR, '. $1');
