...
function customJaroWinkler(options, a, b) {
  ...
  if (scalingFactor > 0.25)
    throw Error('talisman/metrics/jaro-winkler: the scaling factor should not exceed 0.25.');

  if (boostThreshold < 0 || boostThreshold > 1)
    throw Error('talisman/metrics/jaro-winkler: the boost threshold should be comprised between 0 and 1.');
